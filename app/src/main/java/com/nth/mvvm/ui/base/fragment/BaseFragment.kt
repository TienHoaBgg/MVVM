...
abstract class BaseFragment : Fragment(), ViewFragment {
    ...
    override fun onCreateViewControl(
        ...
        mBinding = if (inflater == null) {
            val creatInflater = LayoutInflater.from(context);
            DataBindingUtil.inflate(creatInflater, getLayoutMain(), container, false)
        } else {
            DataBindingUtil.inflate(inflater, getLayoutMain(), container, false)
        }
    ...
    fun appDatabase() = (activity?.applicationContext as HoleApplication).appDatabase()
    fun schedule() = (activity?.applicationContext as HoleApplication).schedule()
